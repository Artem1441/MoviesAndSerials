import { moviesGenres, moviesComainGenres } from "../defaultData/genres";


export const moviesLogic = (item, value, setFits = false, setData = false) => {
  if (value === "false") {
    return false;
  }
  ...
  for (let i = 0; i < item.genre_ids.length; i++) {
    if (
      ...
      (JSON.parse(value).includes(item.genre_ids[i]) ||
        (other && moviesComainGenres.includes(item.genre_ids[i])))
    ) {
      fits = fits + 50;
  ...
  if (JSON.parse(value).sort().join(",") === moviesComainGenres.sort().join(",")) {
    fits = fits + 19;
  }
